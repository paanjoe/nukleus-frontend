  ...
  const fetchRole = async () => {
    ...
    try {
      const response = await fetch(
        `${apiBaseUrl}/entityrole?userid=${userId}`,
        {
          headers: {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {loading === true ? (
        ...
        <>
          {roleData.filter(
            (role) => role.Role === "Read" && role.IsActive === false
          ).length > 0 ? (
            <div
              className="p-4 mb-4 text-sm text-white bold text-center rounded-lg bg-red-50 dark:bg-red-800 dark:red-green-400"
              role="alert"
            >
              <span className="font-medium">
                You do not have view permission! Please go to user management to
                modify roles.
              </span>
            </div>
          ) : (
            <>
              {roleData.filter(
                (role) => role.Role === "Create" && role.IsActive === false
              ).length > 0 ? (
                <>
                  <button
                    type="button"
                    className="text-center w-full mb-6 focus:outline-none text-white bg-gray-700 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-gray-600  dark:focus:ring-gray-900"
                    disabled={true}
                  >
                    ...
                    {filteredData.map((item) => (
                      <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td className="flex items-center px-6 py-4">
                          {roleData.filter(
                            (role) =>
                              role.Role === "Update" && role.IsActive === false
                          ).length > 0 ? (
                            <>
                              <button
                                className="ont-medium text-gray-600 dark:text-gray-500 "
                                disabled={true}
                              >
                                Edit
                              </button>
